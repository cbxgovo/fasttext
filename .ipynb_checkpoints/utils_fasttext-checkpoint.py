...
def build_vocab(file_path, tokenizer, max_size, min_freq):
    ...
    with open(file_path, 'r', encoding='UTF-8') as f:
        for line in tqdm(f):
            ...
            if not lin:
                continue
            content = lin.split('<xumh>')[0]
            # 使用 rsplit 拆分，确保即使文本中包含制表符\t也不会影响标签分离
            # content, _ = lin.strip().rsplit('\t', 1)
    ...
    def load_dataset(path, pad_size=32):
        ...
        with open(path, 'r', encoding='UTF-8') as f:
            for line in tqdm(f):
                ...
                if not lin:
                    continue
                content, label = lin.split('<xumh>')
                # 使用 rsplit('\t', 1) 来只按最后一个 \t 分隔，从而避免文本中出现多个 \t 时出错。
                # content, label = lin.strip().rsplit('\t', 1)
...
class DatasetIterater(object):
    ...
    def __init__(self, batches, batch_size, device):
        ...
        if len(batches) == 0:
            raise ValueError("No data in batches.")
